...
# Original Dockerfile ends. VOLUME directive removed as it does not allow to chown VOLUME directory.
...
RUN [ "x${uid}" = "x" ] || addgroup -g ${uid} -S postgres
RUN [ "x${uid}" = "x" ] || adduser -D -G postgres -u ${uid} -S postgres
